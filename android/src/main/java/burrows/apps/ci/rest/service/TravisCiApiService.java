...
public interface TravisCiApiService {
  ...
  @GET("/repos") List<Repository> getRepos();

  /* curl -v -H "Accept: application/vnd.travis-ci.2+json" \
  https://api.travis-ci.org/repos?=cs-interview-questions */
  @GET("/repos") List<Repository> getRepos(@Query("search") String search);

  /* curl -v -H "Accept: application/vnd.travis-ci.2+json" \
  https://api.travis-ci.org/repos/1123284 */
  @GET("/repos/{repository.id}") Repository getRepo(@Path("repository.id") long id);

  /* curl -v -H "Accept: application/vnd.travis-ci.2+json" \
  https://api.travis-ci.org/repos/jaredsburrows/cs-interview-questions */
  @GET("/repos/{repository.slug}") Repository getRepo(@Path("repository.slug") String slug);

  /* curl -v -H "Accept: application/vnd.travis-ci.2+json" \
  https://api.travis-ci.org/repos/member?=jaredsburrows */
  @GET("/repos") List<Repository> getUserRepos(@Query("member") String member);
}
