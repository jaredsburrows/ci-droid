...
abstract class BaseAdapter<T, VH extends RecyclerView.ViewHolder> extends RecyclerView.Adapter<VH> {
  ...
  public boolean addAll(List<T> collection) {
    boolean added = data.addAll(collection);
  ...
  public T set(int location, T object) {
    T insertedObject = data.set(location, object);
   ...
   */
  public boolean remove(int location, T object) {
    boolean removed = data.remove(object);
  ...
  public T remove(int location) {
    T removedObject = data.remove(location);
    notifyItemRemoved(location);
  ...
  public List<T> getSelectedList() {
    ...
    for (int i = 0; i < selectedItems.size(); i++) {
      T model = getList().get(selectedItems.keyAt(i));
      list.add(model);
    }
    return list;
  }
